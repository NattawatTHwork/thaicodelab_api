...
public class EquipmentTransactionDetail
{
    ...
    [Required(ErrorMessage = "Operator Return User ID is required")]
    [Range(1, int.MaxValue, ErrorMessage = "Operator Return User ID must be a valid ID")]
    public int operator_return_user_id { get; set; }

    [Required(ErrorMessage = "Return User ID is required")]
    [Range(1, int.MaxValue, ErrorMessage = "Return User ID must be a valid ID")]
